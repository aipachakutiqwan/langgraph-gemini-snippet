...
async def double_texting():
    ...
    print("Wait until the second run completes")
    ...
    print(
        (await client.runs.get(thread["thread_id"], interrupted_run["run_id"]))[
            "status"
        ]
    )
    # ROLLBACK
